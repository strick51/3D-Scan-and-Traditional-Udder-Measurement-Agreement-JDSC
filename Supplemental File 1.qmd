...
#read in data
```{r}
#measurements collected from 3D images using 3-Matic Software
udder3d <- read_csv("Supplemental File 2.csv")
#exported cow events from farm's herd management software
health <- read_csv("Supplemental File 3.csv")
#combined traditional measurements collected with measuring tape
measures <- read_csv("Supplemental File 4.csv")
#test day information on individual cow milk quality
tday <- read_csv("Supplemental File 5.csv")
```


#summarizing cow milk quality 
```{r}
#evaluating cows available for enrollment
tday1 <- tday %>%
...
#filtering out cows excluded from study to summarize pre-study milk quality
tday1_sum <- tday1 %>%
...
#wrangle health data to select study cows
```{r}
health2 <- health %>%
  filter(EART %in% c(5245, 
                     5246, 
...
#wrangle traditional measurements
```{r}
measures2 <- measures %>%
  mutate(
...
#converting 3D measurements from dm to cm and more wrangling
```{r}
udder3d_02 <- udder3d %>%
  mutate(
...
```
#agreement for vertical obs B x 3D
```{r}
vert_3db <- measures5 %>%
  filter(Initials == "LJ")  %>%
  select(ID, Vertical, vertical_3d)
...
#left quarter obs x 3D reliability
...
#left quarter obs x 3D agreement 
```{r}
